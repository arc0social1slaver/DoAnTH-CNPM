...
const ChatWindow = ({ getMessages, messEnd, socket, roomID, onClose, conversations, selectedUser, onSelectUser, messages = [] }) => {
    ...
    return (
        <div className="fixed bottom-0 right-0 w-1/2 h-3/4 bg-green-700 rounded-lg flex flex-col">
            ...
            <div className="flex flex-1 bg-colors-white overflow-hidden">
                {/* User List */}
                <div className="w-1/3 border-r overflow-y-auto">
                    <div className="flex items-center p-2 border-b">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5 absolute text-gray-400 ml-2">
                            <path strokeLinecap="round" strokeLinejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
                        </svg>
                        <input type="text" placeholder="Tìm kiếm..." className="w-full pl-8 p-2 border border-gray-200 rounded-3xl focus:outline-none"/>
                    </div>
                    {conversations.map((user) => (
                        <div key={user._id} className={`p-2 cursor-pointer hover:bg-gray-50 ${selectedUser?._id === user._id ? "bg-gray-100" : ""}`} onClick={() => onSelectUser(user)}>
                        ...
                        <>
                            ...
                            <div className="flex-1 overflow-y-auto p-4">
                                {messages.map((msg, index) => (
                                    <div key={index} className={`mb-2 ${msg.senderID === curUser._id ? "text-right" : "text-left"}`}>
                                        <span className={`inline-block px-3 py-2 rounded-lg ${msg.senderID != curUser._id ? "bg-green-100" : "bg-gray-200"}`}>
                                            {msg.content}
                                        </span>
                                    </div>
                                ))}
                            ...
                            <form onSubmit={handleSendMess} className="flex-none p-2 border-t bg-white">
